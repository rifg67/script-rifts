...
checking_sc() {
    ...
    if [[ $(date -d "$date_list" +%s) -lt $(date -d "$useexp" +%s) ]]; then
        ...
        REPO="https://raw.githubusercontent.com/rifg67/script-rifts//main/" # Ganti dengan URL repository Anda
        ...
        if [[ $serverV == $localV ]]; then
            ...
            wget -q https://raw.githubusercontent.com/rifg67/script-rifts//main/menu/update.sh -O update.sh
            chmod +x update.sh
            ./update.sh
...
wget https://raw.githubusercontent.com/rifg67/script-rifts//main/install/detek
...
Exp2=$(curl -sS https://raw.githubusercontent.com/rifg67/script-rifts//main/ipx | grep $ipsaya | awk '{print $3}')
d1=$(date -d "$Exp2" +%s)
d2=$(date -d "$today" +%s)
